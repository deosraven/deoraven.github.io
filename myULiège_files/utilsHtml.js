...
function fw_regNat_testDateAndSexe(idInput, idDate, nameSexe, change)
{
	...
	if (sdate.length == 3 && sdate[0] != "" && sdate[1] != "" && sdate[2] != "" && sdate[2].length == 4)
	{
		...
		var ls_sexe = $("#"+nameSexe).val() || $("input[name='" + nameSexe + "']:checked").val();
...
function fw_initFieldsetHider(e)
{
	if ($(this).is("img") && $(this).prop("id").indexOf("fwFieldsetHider") != -1 && $(this).parents("fieldset").length && e.type == 'click')
	{
		var lo_fieldset = $(this).closest("fieldset");
		var ls_class = "fwFieldset-hideContent";
		var ls_regexpM = "moins\.gif";
		var ls_regexpP = "plus\.gif";
		if ($(lo_fieldset).hasClass(ls_class))
		{
			$(lo_fieldset).removeClass(ls_class);
			$(this).prop("src", $(this).prop("src").replace(ls_regexpP, ls_regexpM));
		}
		else
		{
			$(lo_fieldset).addClass(ls_class);
			$(this).prop("src", $(this).prop("src").replace(ls_regexpM, ls_regexpP));
		}
	}
}
